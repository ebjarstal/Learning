...
bool pointInTriangle(const Point3D &p, const Triangle &t) {
    ...
    // Cross product between each side of the triangle and the vectors formed
    // with point 'p'
    const auto v1v4 = v1.crossProduct(v4);
    const auto v2v5 = v2.crossProduct(v5);
    const auto v3v6 = v3.crossProduct(v6);

    // If the vectors from the cross products are all codirectional with one
    // another then 'p' is in 't'
